...
Local_To_Gps_Trans::Local_To_Gps_Trans() : nh("~"), flag(false) {
  ...
  sub_ori = nh.subscribe<gps_to_local::fcc>(
      "/fcc_messages", 10, &Local_To_Gps_Trans::OriginCallBack, this);
...
void Local_To_Gps_Trans::EnuToWgs(double &lat, double &lon, double &h,
  ...
  double e, b2, e2, ep, r, r2, E2, F, G, c, s2, P, Q, ro, tmp, U, V, zo, height,
      temp;
  e = sqrt(1 - (b / a) * (b / a));
  b2 = b * b;
  e2 = e * e;
  ...
  Q = sqrt(1 + 2 * e2 * e2 * P);
  ro = -(P * e2 * r) / (1 + Q) +
       sqrt((a * a / 2) * (1 + 1 / Q) - (P * (1 - e2) * z2) / (Q * (1 + Q)) -
            P * r2 / 2);
  tmp = (r - e2 * ro) * (r - e2 * ro);
  U = sqrt(tmp + z2);
  V = sqrt(tmp + (1 - e2) * z2);
...
void Local_To_Gps_Trans::LocalToGpsCallBack(
  ...
  if ((ros::Time::now() - last_publish_time).toSec() >= (1.0 / publish_rate_)) {
    ...
    std::cout << "East:" << ans_x << "   North:" << ans_y << "   Up:" << ans_z
              << std::endl;
    double lat, lon, alt;
    // ENU_to_GPS
