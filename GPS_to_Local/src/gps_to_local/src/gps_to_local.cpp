...
Gps_Local_Trans::Gps_Local_Trans() : nh("~"), flag(false) {
  ...
  */
  //订阅航点经纬高信息
  sub_1 = nh.subscribe<gps_to_local::waypoints>(
      "/waypoints_messages", 10, &Gps_Local_Trans::GpsToLocalCallBack, this);

  //订阅起始点经纬高
  sub_2 = nh.subscribe<gps_to_local::fcc>(
      "/fcc_messages", 10, &Gps_Local_Trans::OriginPointCallBack, this);
  //发布航点信息
  pub_ego = nh.advertise<gps_to_local::vectormessage>("/publish_points", 10);
}

void Gps_Local_Trans::EnuToXyz(double north_, double east_, double up_,
                               double roll, double pitch, double yaw, double& x,
                               double& y, double& z) {
  // 将角度转为弧度
  double rollRad = roll;
  ...
  x = R_ENU_to_local[0][0] * east_ + R_ENU_to_local[0][1] * north_ +
      R_ENU_to_local[0][2] * up_;
  y = R_ENU_to_local[1][0] * east_ + R_ENU_to_local[1][1] * north_ +
      R_ENU_to_local[1][2] * up_;
  z = R_ENU_to_local[2][0] * east_ + R_ENU_to_local[2][1] * north_ +
      R_ENU_to_local[2][2] * up_;
  return;
}

void Gps_Local_Trans::OriginPointCallBack(
    const gps_to_local::fcc::ConstPtr& originmsg) {
  if (!flag) {
    origin_points = *originmsg;
    ROS_INFO("Originpoint_CallBack reveived message!!");
    std::cout << "origin" << origin_points.latitude << " "
              << origin_points.longitude << " " << origin_points.altitude
              << std::endl;
    flag = true;
  }
  // return;
...
void Gps_Local_Trans::WgsToEnu(double lat, double lon, double h, double lat0,
                               double lon0, double h0, double& enu_x,
                               double& enu_y, double& enu_z) {
  ...
  enu_z = cos_lambda0 * cos_phi0 * xd + cos_lambda0 * sin_phi0 * yd +
          sin_lambda0 * zd;
  return;
}

void Gps_Local_Trans::GpsToLocalCallBack(
    const gps_to_local::waypoints::ConstPtr& msg) {
  gps_to_local::waypoints current_point = *msg;
  //转为东北天
  double east, north, up;

  WgsToEnu(current_point.latitude, current_point.longitude,
           current_point.altitude, origin_points.latitude,
           origin_points.longitude, origin_points.altitude, east, north, up);
  // 输出东北天坐标
  ...
  EnuToXyz(north, east, up, origin_points.roll, origin_points.pitch,
           static_cast<double>(origin_points.orientation_uav),
  ...
  outfile.open("/home/kao/GPS_to_Local/src/gps_to_local/src/output.txt",
               std::ios_base::app);
  if (outfile.is_open()) {
